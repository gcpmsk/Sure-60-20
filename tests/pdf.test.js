...
import {parseQuestions,parseAnswers,validatePdf} from '../src/pdf.js';
...
test('PDF upload validates type, signature, empty files and size',async()=>{
  const pdf=new File(['%PDF-1.4\nexample'],'notes.pdf',{type:'application/pdf'});
  await validatePdf(pdf);
  await assert.rejects(validatePdf(new File(['text'],'notes.txt')),/Choose a PDF/);
  await assert.rejects(validatePdf(new File([],'notes.pdf')),/empty/);
  await assert.rejects(validatePdf(new File(['not pdf'],'notes.pdf')),/not a valid PDF/);
  await assert.rejects(validatePdf({name:'large.pdf',size:16*1024*1024}),/15 MB/);
});
